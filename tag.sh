...
if [ $# -eq 0 ]; then
    echo "$(tput setaf 1)$(tput bold)Please fill arguments [major/minor/dev/patch]$(tput sgr 0)"
    exit 0
elif [ $1 == "major" ]; then
    echo "$(tput setaf 6)$(tput bold)Update tag version $(tput setaf 4)$(tput bold)x.x.x$(tput sgr 0)"
    currentVersion
    ...
    echo "$(tput setaf 6)$(tput bold)Update tag version $(tput setaf 4)$(tput bold)x.x.minor$(tput sgr 0)"
    currentVersion
    ((increment[2]++))
